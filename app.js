...
app.post('/api/upload', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        // Find missing IDs
        const foundIds = foundCves.map(cve => cve.cveMetadata.cveId);
        const missingIds = uniqueIds.filter(id => !foundIds.includes(id));

        res.json({
            totalFileIds: uniqueIds.length,
            resolvedLocal: foundCves,
...
async function startServer() {
    ...
    app.listen(PORT, () => {
        console.log(`[Server] CVE Browser running on http://localhost:${PORT}`);
